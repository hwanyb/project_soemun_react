export const HomeImgArr = [
    {
      id: 1,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/sec4_img01.jpg?raw=true",
    },
    {
      id: 2,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/sec4_img02.jpg?raw=true",
    },
    {
      id: 3,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/sec4_img03.jpg?raw=true",
    },
    {
      id: 4,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/sec4_img04.jpg?raw=true",
    },
    {
      id: 5,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/sec4_img05.jpg?raw=true",
    },
    {
      id: 6,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/sec4_img06.jpg?raw=true",
    },
    {
      id: 7,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/sec4_img07.jpg?raw=true",
    },
    {
      id: 8,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/sec4_img08.jpg?raw=true",
    },
    {
      id: 9,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/sec4_img09.jpg?raw=true",
    },
    {
      id: 10,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/shrimp.jpg?raw=true",
    },
    {
      id: 11,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/tangsu.jpg?raw=true",
    },
    {
      id: 12,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/ttoeokbok.jpg?raw=true",
    },
    {
      id: 13,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/gogi.jpg?raw=true",
    },
    {
      id: 14,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/kabe.jpg?raw=true",
    },
    {
      id: 15,
      url: "https://github.com/hwanyb/project_seomun/blob/master/img/kkochi.jpg?raw=true",
    },
  ];

export const subwayArr = [
    {
        name: "서문시장역",
        subwayId: "DGS317",
        line: 3
    },
    {
        name: "달성공원역",
        subwayId: "DGS316",
        line: 3
    },
    {
        name: "청라언덕역",
        subwayId: "DGS318",
        line: 3
    },
    {
        name: "청라언덕역",
        subwayId: "DGS229",
        line: 2
    },
];

export const busArr = [
    {
        name: "서문시장5지구앞",
        url: "http://kko.to/-sYfG6w89o",
    },
    {
        name: "동산의료원앞1",
        url: "http://kko.to/wa37WX3Gdt"
    },
    {
        name: "서문시장",
        url: "http://kko.to/FqZCIyqtQA"
    },

];

export const foodArr = [
    {
        name: '스테이크 새우꼬치',
        tag: '스테이크',
        desc: '',
        location: '동산상가',
        image: 'http://www.nightseomun.com/upload/img/ya44773_01_1609722489.jpg',
        menu: [
            {
                title: '스테이크 새우꼬치',
                price: '3000원'
            },
            {
                title: '새우꼬치',
                price: '3000원'
            },
            {
                title: '스테이크꼬치',
                price: '3000원'
            },
        ]

    },
    {
        name: '야채뚱땡 삼겹말이',
        tag: '육류튀김류',
        location: '대신',
        desc: '즉석철판에 구운 삼겹살야채말이!특제소스에불맛을낸두가지맛! 매콤한야채뚱땡~달콤한야채뚱땡~~',
        image: 'http://www.nightseomun.com/upload/img/ya13650_03_1488249125.jpg',
        menu: [
            {
                title: '야채삼겹말이',
                price: '4000원'
            },
            {
                title: '불야채삼겹말이',
                price: '4000원'
            },
        ]
    },
    {
        name: '차돌박야끼소바',
        tag: '육류튀김류',
        location: '대신',
        desc: '쫀득 쫀득 달콤 새콤 탕수육 입니다^^',
        image: 'http://www.nightseomun.com/upload/img/ya54852_01_1490267119.jpg',
        menu: [
            {
                title: '차돌박야끼소바',
                price: '8000원'
            },
            {
                title: '탕수육',
                price: '5000원'
            },
        ]
    },
    {
        name: '이모네 막창',
        tag: '내장류',
        location: '대신',
        desc: '파 향 가득한 이모네막창',
        image: 'http://www.nightseomun.com/upload/img/ya50862_02_1647619729.jpg',
        menu: [
            {
                title: '대파막창',
                price: '7000원'
            },
            {
                title: '소금막창',
                price: '8000원'
            },
            {
                title: '양념막창',
                price: '9000원'
            },
        ]
    },
    {
        name: '반반막창',
        tag: '내장류',
        location: '대신',
        desc: '고소한 막창구이와 매콤한 중독성 있는 불막창',
        image: 'http://www.nightseomun.com/upload/img/ya58665_03_1515160455.jpg',
        menu: [
            {
                title: '반반막창',
                price: '5900원'
            },
            {
                title: '막창구이',
                price: '5900원'
            },
            {
                title: '불막창',
                price: '5900원'
            },
        ]
    },
    {
        name: '팟타이',
        tag: '면류',
        location: '동산상가',
        desc: '팟타이 Pad thai 泰国式炒米粉',
        image: 'http://www.nightseomun.com/upload/img/ya70359_03_1515308335.jpg',
        menu: [
            {
                title: '팟타이',
                price: '6000원'
            },
            {
                title: '새우치킨팟타이',
                price: '7000원'
            },
            {
                title: '소고기팟타이',
                price: '7900원'
            },
        ]
    },
    {
        name: '철판아이스크림',
        tag: '디저트류',
        location: '대신',
        desc: '즉석에서 바로 만들어드리는 수제 철판 아이스크림',
        image: 'http://www.nightseomun.com/upload/img/ya26383_03_1525622190.jpg',
        menu: [
            {
                title: '초코 아이스롤',
                price: '5000원'
            },
            {
                title: '딸기 아이스롤',
                price: '5000원'
            },
            {
                title: '그린티 아이스롤',
                price: '5000원'
            },
            {
                title: '쿠앤크 아이스롤',
                price: '5000원'
            },
        ]
    },
    {
        name: '카베츠야끼',
        tag: '면류',
        location: '대신',
        desc: '즉석에서 철판에 볶는 일본 정통요리!',
        image: 'https://hwanyb.github.io/project_seomun/img/kabe.jpg?raw=true',
        menu: [
            {
                title: '카베츠야키',
                price: '5000원'
            },
            {
                title: '오꼬노미야끼',
                price: '5000원'
            },
            {
                title: '야끼소바',
                price: '5000원'
            }
        ]
    },
];


export const mapArr = [
    {
        name: "서문시장",
        mapUrl: "http://kko.to/QRhOiymZFe",
        mapImgUrl: "http://t1.daumcdn.net/roughmap/imgmap/22610f4b8d20243120ba42b8fc692b609847ea32151357ce9904b8ed48d3a23c",
        loadViewUrl: "http://kko.to/PgZmBEDTGA",
    },
    {
        name: "동산상가",
        mapUrl: "http://kko.to/7Z3trbzC50",
        mapImgUrl: "http://t1.daumcdn.net/roughmap/imgmap/1d607f7693aa2ee46602b7e6d96e08b6f676097050db9f586086453af216be82",
        loadViewUrl: "http://kko.to/vpr3AX1a-A",
    },
    {
        name: "대신",
        mapUrl: "http://kko.to/imgjGL_uoh",
        mapImgUrl: "http://t1.daumcdn.net/roughmap/imgmap/7872e8e008f7fd86892ee9af764816346268616cc71d4b76d66ae5a58dc5c8f5",
        loadViewUrl: "http://kko.to/P74am_0sH_",
    }
];


export const noticeArr = [
    {
        title: "서문·칠성야시장 매대운영자 모집공고",
        date: "2022-03-24",
        body: `<서문·칠성야시장 매대운영자 모집공고 >

        전국을 대표하는 야간관광지 「대구 서문·칠성 야시장」에서 뛰어난 열정을 펼칠 수 있는    창의적이고 역량이 뛰어난 매대 운영자를 모집하오니 많은 참여를 바랍니다.
      
       
      
       
      
      ❍   1. 모집분야
      
      - 음식매대
      
      야시장 영업에 적합한 특색 있는 퓨전·창작요리, 글로벌 음식, 지역을 대표하는
      
      인지도 있는 향토 먹거리
      
              ※ 매주 화요일은 정기휴무이며 기후, 재해, 야시장 운영 정책에 따라 영업시간 등이 변경될 수 있음
      
      - 프리마켓 운영업체
      
      핸드메이드, 공예품, 아트상품, 생활잡화, 재능판매 등 20~30여개 프리마켓 셀러를 관리하는 업체
      
      ※ 금~일, 공휴일 운영하며 영업시간은 야시장 영업시간과 동일
      
      - 대학매대
      
      대구·경북권 식품관련학과 대학생들의 야시장에서 현장경험을 쌓기 위한 재학생 및 1년 이내 졸업생(학교 추천서)
      
      ※ 토~일, 공휴일 운영하며 영업시간은 야시장 영업시간과 동일
      
       
      
      ❍   2. 모집인원
      
      - 음식매대 40명(서문야시장 20명, 칠성야시장 20명)
      
      - 프리마켓 운영업체 2개 업체(서문야시장 1개, 칠성야시장 1개)
      
      - 대학매대 4명(서문야시장 2명, 칠성야시장 2명)
      
        
      
      3 응시자격
      
      - 거주지 제한 없음
      
      - 만 18세 이상 ~ 만 60세 이하(1962. 1. 1. ~ 2003. 12. 31. 출생자)
      
      - 신청자(배우자)가 직접 운영
      
      - 신청자 본인 사업자등록증 발급에 결격 사유가 없어야 함
      
      - 1가구(직계가족) 1매대 이상 신청할 수 없음
      
      ※ 서문, 칠성야시장 퇴점자 중 2021년 3월 이전에 퇴점자는 지원 가능
      
       
      
      4. 지원자 혜택
      
      - 3개월간 관리비 지원(개별 전기요금 제외)
      
      - 메뉴 및 밀키트 개발 컨설팅
      
      - 매대 상호, 홍보판 등 VMD지원(대학매대)
      
       
      
      5. 신청방법
      
      - 지원을 희망하는 야시장(서문, 칠성 야시장 중 택 1) 지원서 작성
      
      - 지원서 제출방법 : 홈페이지, 이메일, 방문 접수 / 문의처 : 053) 621-1985~6
      
      - 홈페이지 : 서문야시장( http://www.nightseomun.com ), 칠성야시장( http://7starnm.com )`
    },
    {
        title: "서문야시장 푸드트럭 셀러 모집",
        date: "2022-08-08",
        body: `대구 서문야시장 에서 푸드트럭 셀러를 모집하오니 

        많은 관심과 참여바랍니다.
        
         
        
        1. 운영 개요
        
         ○ 운영기간 : 2022. 9. ~ 2023. 12.(혹한기 1, 2월 휴장) 
        
         ○ 운영시간 : 금·토·일 19:00 ~ 24:00(운영시간은 변동될 수 있음)
        
           - 명절 연휴 및 공휴일 영업은 협의 후 결정
        
         ○ 장 소 : 서문시장
        
         ○ 모집규모 : 15 ~ 20대
        
         
        
        2. 모집개요
        
         ○ 모집대상 : 2022년 서문야시장 푸드트럭 셀러 희망자
        
         ○ 선발분야 : 음식 품목군별 선정 
        
           ※ 품목 분류군 : 스테이크류, 새우류, 막창·곱창류, 꼬치류, 분식류, 한식류
        
                             아시아음식, 양식, 중식, 일식, 디저트류, 기타
        
         ○ 월 관리비 : 기본 월 60만원 정도(변동 될 수 있음) + 개인 전기료
        
          ※ 영업신고(필수) 시 별도의 등록세·면허세 납부가 필요
        
         
        
        3. 참가자격 및 제한사항
        
          ○ 푸드트럭 소유하고, 음식을 직접 조리 하는 자
        
          ○ 차량규격 : 1톤 이내, 윙바디 개폐 시 높이 3.5m 이하, 가로길이 7m 이하
        
                         푸드트럭 소지자
        
          ○ 푸드트럭 영업신고를 위한 조건을 모두 갖추어야하며, 사업자등록증 발급 가능한 자
        
          ○ 야시장 운영 전(全)기간에 참여 가능한 자
        
          ○ 야시장 내에 직계 가족은 1인만 신청 가능
        
             (신청자 본인의 가족관계증명서 상 표시되는 부모, 배우자, 자녀)
        
         
        
        4. 접수방법
        
         ○ 1단계 : 홈페이지 내 입점신청 → 푸드트럭 입점신청 온라인 시스템 등록 신청
        
         ○ 2단계 : 지원신청서(붙임2), 동의서(붙임3, 4), 영업신고 필요서류, 증빙서류 등 
        
                        제출서류를 이메일 또는 우편 접수
        
         ○ 3단계 : 최종 신청 완료
        
         
        
        5. 문의 : 053)621-1985~6  `
    },
    {
        title: "서문야시장 임시휴장 안내(7. 18.)",
        date: "2022-07-18",
        body: `서문야시장 임시휴장

 

        금일 2022년 7월 18일 서문야시장은 우천으로 안전 대비를 위하여 임시휴장합니다.
        
        
        감사합니다.
        
        `
    },
    {
        title: "임시휴장 안내(2022.07.06.)",
        date: "2022-07-06",
        body: `서문야시장 임시휴장

 

        금일 2022년 7월 6일 서문야시장은 우천으로 안전 대비를 위하여 임시휴장합니다.
        
        
        감사합니다.
        
        `
    },
    {
        title: "2022 야시장 매대 운영자 모집 품평회 심사 대상 명단 공지",
        date: "2022-04-18",
        body: `
        2022 야시장 매대운영자 모집 품평회 심사 대상 발표 및 품평회 설명 자료 게시 합니다 .

 

첨부파일 참조 (심사 대상 명단 , 응시조 확인 및 일정 , 참가자 설명자료 반드시 확인 )

 


❍ 매대운영자 모집 품평회

 

가 . 대 상 : 식품분야 품평회 심사 대상으로 개최

 

나 . 일 시 : 2022. 04. 22.(금) 14:00분~

 

다 . 장 소 : 수성대학교 강산관 5층(대구 수성구 달구벌대로 528길 15)

 

라 . 준비물 : 신분증 , 조리기구 , 방역마스크 등 (첨부파일 품평회 설명 자료 반드시 참조 )

 


마 . 심사방법

 

- 품평회를 통한 전문가 심사로 최종 선정

 

문의 070-7731-2040 대구전통시장진흥재단 상권육성팀

 

※ 시험당일 주차권은 배부되지 않으니 이점 참고해주시기 바랍니다.
        `
    }
]